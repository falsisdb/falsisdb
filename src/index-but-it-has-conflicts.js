...
class database extends EventEmitter{
  constructor(construct) {
    ...
    if(!fs.existsSync(this.backup) || !fs.lstatSync(this.backup).isFile()) {
      ...
      if(this.btype == "txt"){
        writeFileWithDirs(`Backup Oluşturuldu | ${formatDate(new Date())} | {}`, this.backup)
      }
    }else {
    }
    ...
    if(this.backup != false) {
      ...
      if(!fs.existsSync("./falsisdb/backupData.json") || !fs.lstatSync("./falsisdb/backupData.json").isFile()){
        writeFileWithDirs("{}", "./falsisdb/backupData.json")
  ...
  this.lastBackupData.backupDB.set("backupcount", "0")      
        ...
        this.backupdata = Object.entries(JSON.parse(fs.readFileSync(this.backup, "utf-8")))[0][1].data
        //console.log(this.backupkeys)
      }
    }
   /* if(this.backup != false){
      process.on("exit", ()=> {
        console.log(this.backupcount)
this.lastBackupData.backupDB.set("backupcount", this.backupcount)
      })  
    }*/
