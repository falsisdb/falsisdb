...
class YAMLBackup extends EventEmitter {
  constructor(opts) {
    ...
    if(!fs.existsSync("./falsisdb/backupData.yaml") || !fs.lstatSync("./falsisdb/backupData.yaml").isFile()){
      writeFileWithDirs("{}", "./falsisdb/backupData.yaml")
    }
    this.lastBackupData.backupDB = new db({
      filePath:"./falsisdb/backupData.yaml"
    })
    this.getBackupData = () => {
        let res = {}; Object.entries(YAML.parse(fs.readFileSync(this.path, "utf-8"))).map(x=>Object.entries(x[1].data)).forEach(x=> {
          x.forEach(u => {
            res[u[0]] = u[1]
          })
    ...
    /*this.lastBackupData.backupDB.ayarla = (key,value) => {
      ...
      if(fs.readFileSync(this.path,"utf-8") == "") {
        fs.writeFileSync(this.path, "{}","utf-8")
          ...
          if(fs.readFileSync("./falsisdb/backupData.yaml","utf-8") == "{}") {
          this.lastBackupData.backupDB.set("BackupData",[])/*this.lastBackupData.backupDB.set("backupCount","0")
